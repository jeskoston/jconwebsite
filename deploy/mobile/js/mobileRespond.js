...
$('document').ready(function()
{
	...
	var logo=$("#centeredImage");//JC logo
  ...
  */
    ...
    function resizeHandler(windowWidth)
    {
		...
		//VVVVV MMMMMOOOOOBBBBBIIILLLLLEEEEE Mobile Devices  MMMMMOOOOOBBBBBIIILLLLLEEEEE VVVVV
		else if (windowWidth <= 1100 && windowWidth >= 981)
        {
			imgSize = mobXxl; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 980 && windowWidth >= 821)
        {
			imgSize = mobXl; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 820 && windowWidth >= 741)
        {
           	imgSize = mobLarge; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 740 && windowWidth >= 641)//At this point the hieght seperations starts to be an issue
        {
		...
		else if (windowWidth <= 640 && windowWidth >= 581)//new breakpoiint 
        {
			imgSize = mobSmall; 
			iptSize = smlIpt;
			updateImage(imgSize); 
	...
	else if (windowWidth <= 580 && windowWidth >= 501)
        {
			imgSize = mobSmaller; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 500 && windowWidth >= 411)
        {
			imgSize = mobXs; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 410 && windowWidth >= 311)
        {
			imgSize = mobXxs; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		else if (windowWidth <= 310 && windowWidth >= 200)
        {
			imgSize = tiny; 
			iptSize = smlIpt;
			updateImage(imgSize); 
	    ...
	    function onResizeHandler()
    {
		...
		else if (windowWidth <= 740 && windowWidth >= 641)
        {
			imgSize = mobMedium; 
			iptSize = smlIpt;
			updateImage(imgSize); 
		...
		//580 should be the breakpoint
		else if (windowWidth <= 640 && windowWidth >= 581)
        {
			imgSize = mobSmall; 
			iptSize = smlIpt;
			updateImage(imgSize); 
